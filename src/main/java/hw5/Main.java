...
public class Main {
    ...
    public static void formula1(float[] arr) {
        ...
        for (int i=0; i<arr.length; i++){
            arr[i] = (float) ((float) (arr[i] * Math.sin(8.2f + i/5)) * Math.cos(0.2f + i/5)* Math.cos(8.4f + i/2));
        }
    ...
    public static void formula2(float[] arr) {
        ...
        for (int i=0; i< a2.length; i++){
            a1[i] = (float) (a2[i] * Math.sin(0.2f + (i + HALF) /5) * Math.cos(0.2f + (i + HALF)/5) * Math.cos(0.4f + (i+HALF/2)));
        }
